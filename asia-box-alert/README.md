...
- **asia_box_lines**：画线策略（默认 M15；窗口「画线周期」可改 H1）
  - 图上会叠加 **Entry / SL / TP** 三条线（绿/红/金）和价格标签
- **asia_box_lines_h1**：画线策略（强制小时级；也可用「画线周期」代替）
- **asia_box_dual_lines_hwr**：双策略（画线 + 高胜率）；画线腿跟随「画线周期」（默认 M15，可改 H1），高胜率仍为 M15 盒子
- 手数可在窗口选 **0.02 / 0.03 / 0.05 / 0.07**；画线周期可选 **M15 / H1**
...
- **提醒自动记**（默认开）：入场弹窗时自动写入开仓；网格全平等平仓提醒会尝试平最近一笔未平仓。仍可用 **手动补记 / 平最近一笔**
- **关闭全部弹窗**：提醒改为置顶小窗（不卡住主界面），主窗口或任意提醒窗上可一键关掉全部已弹出的提醒
- **北京时间 02:00–09:00** 自动静默（界面仍更新，不弹新窗）
- **今日复盘**：当日笔数与盈亏合计；本地 `trades.json` / `trades.csv`，更新程序不覆盖
