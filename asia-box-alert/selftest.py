...
def run_selftest(popup: bool = False) -> int:
    ...
    from alerts import close_all_alert_windows, open_alert_count
    from tzutil import BEIJING as _BJ

    quiet = _dt(2026, 8, 17, 3, 30, tzinfo=_BJ)
    day = _dt(2026, 8, 17, 15, 0, tzinfo=_BJ)
    ok_q, reason_q = alert_gate_open(quiet)
    ok_d, _ = alert_gate_open(day)
    if (not ok_q) and "静默" in reason_q and ok_d:
        print("[OK] 02:00–09:00 静默，白天可提醒")
        ok += 1
    else:
        print(f"[FAIL] 提醒门控异常: quiet={ok_q}/{reason_q} day={ok_d}")
        fail += 1
    # 无 GUI 时 close_all 应安全返回 0
    if close_all_alert_windows() == 0 and open_alert_count() == 0:
        print("[OK] 关闭全部弹窗在无窗口时安全")
        ok += 1
    else:
        print("[FAIL] close_all_alert_windows 异常")
        fail += 1
